...
生产服务器若使用外部 cron 触发 GitHub Actions，请直接部署仓库内的
`deploy/trigger_huohua.sh`。脚本默认调用当前仓库 `sweetcornna/fire` 的正式
`schedule.yml`（`main` 分支）；该工作流会再检出当前的 `dev` 修复版本，并等待
工作流完成后检查结果。`schedule_dev.yml` 仅保留给手动调试使用。

两条工作流的真实发送都必须从仓库默认分支触发，实际执行代码仍检出 `dev`。
这使每日状态缓存既使用同一前缀，也处于同一分支范围；其他分支仅允许文案预览。
触发脚本会读取仓库的默认分支，显式传入不同的 `HUOHUA_GITHUB_REF` 会被拒绝。

发送结果不明确的目标会记录为 `pending`，不会自动再次提交，也不会算作完成。
核验步骤和状态文件格式见[源码部署说明](源代码部署说明.md)。日志附件包含当次状态
备份；工作流在取消时也会尝试保存已有状态，但强制终止 runner 或缓存服务故障仍可能
使保存失败。发生这种情况时先核对已有日志和聊天记录，再决定是否补发。
